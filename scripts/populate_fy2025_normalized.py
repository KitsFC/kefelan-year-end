...
def parse_td_business_visa_transactions() -> list[Transaction]:
    rel = BUS_CC_MD.relative_to(ROOT).as_posix()
    raw = BUS_CC_MD.read_text(encoding="utf-8", errors="replace")
...
def main() -> int:
    ...
    print(f"Wrote {len(docs)} documents, {len(txs)} transactions, {len(allocs)} allocations, {len(assets)} assets")
    return 0
